        ...
        <div className="relative container-custom py-16 md:py-20 w-full">
          <nav aria-label="breadcrumb" className="mb-6">
            <ol className="flex items-center gap-2 text-sm text-white/70 flex-wrap">
              ...
              <li><Link href="/services" className="hover:text-[#0F766E] transition-colors">خدماتنا</Link></li>
              <li><ArrowLeft className="w-3 h-3" /></li>
          ...
          <div className="max-w-3xl">
            ...
            <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black leading-[1.1] mb-6 tracking-tight">
              <span className="text-[#0F766E]">{service.name}</span>
            </h1>
            ...
            {service.priceRange && (
              <div className="mt-8 inline-flex items-center gap-3 bg-white/5 border border-[#0F766E]/30 rounded-2xl px-5 py-3">
            ...
            <div className="grid lg:grid-cols-3 gap-10 lg:gap-16 items-start max-w-6xl mx-auto">
              <div className="lg:col-span-2">
                ...
                <h2 className="text-3xl md:text-4xl lg:text-5xl font-black text-slate-900 mb-6 leading-tight tracking-tight">
                  كل ما تحتاج معرفته عن <span className="text-[#0F766E]">{service.name}</span>
                </h2>
          ...
          <div className="container-custom">
            ...
            <div className="grid md:grid-cols-2 gap-4 max-w-4xl mx-auto">
              {service.features.map((feature, i) => (
                <div key={i} className="flex items-start gap-3 bg-white p-5 rounded-2xl border border-[#E5E7EB] hover:border-[#0F766E] hover:shadow-md transition-all">
                  <div className="w-10 h-10 bg-[#0F766E]/10 text-[#0F766E] rounded-xl flex items-center justify-center shrink-0">
                    <CheckCircle2 className="w-5 h-5" />
                  </div>
                  <p className="text-slate-900 font-semibold text-sm md:text-base pt-1.5">
                    {feature}
                  </p>
                </div>
              ))}
          ...
          <div className="container-custom">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
              {service.benefits.map((benefit, i) => (
                <Card key={i} className="border-[#E5E7EB] hover:border-[#0F766E] hover:shadow-md transition-all bg-white h-full group">
                  <CardContent className="p-6">
                    <div className="w-14 h-14 bg-[#FAF5EE] group-hover:bg-[#0F766E] text-[#0F766E] group-hover:text-white rounded-2xl flex items-center justify-center mb-4 transition-all">
          ...
          <div className="relative container-custom">
            <div className="text-center max-w-2xl mx-auto mb-12">
              ...
              <h2 className="text-3xl md:text-4xl lg:text-5xl font-black mb-4 tracking-tight">
                كيف <span className="text-[#0F766E]">نعمل</span>؟
              </h2>
            </div>

            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5 max-w-6xl mx-auto">
              {service.process.map((step, i) => (
                <div key={i} className="bg-white/5 border border-white/10 rounded-2xl p-6 hover:border-[#0F766E]/50 hover:bg-white/10 transition-all group relative">
                  <div className="absolute -top-4 -right-4 w-12 h-12 bg-[#0F766E] rounded-2xl flex items-center justify-center text-white font-black text-lg shadow-lg">
                    {step.step}
                  </div>
            ...
            <div className="flex flex-wrap justify-center gap-2 md:gap-3 max-w-4xl mx-auto">
              {service.suitableFor.map((item, i) => (
                <div key={i} className="inline-flex items-center gap-2 bg-white border border-[#E5E7EB] hover:border-[#0F766E] hover:bg-[#0F766E] hover:text-white text-slate-900 px-4 py-2 md:px-5 md:py-2.5 rounded-full text-sm font-semibold transition-all cursor-default group">
                  <CheckCircle2 className="w-3.5 h-3.5 text-[#0F766E] group-hover:text-white" />
                  <span>{item}</span>
                </div>
              ))}
            </div>
                  ...
                  <Link key={r.slug} href={`/services/${r.slug}`} className="group">
                    <Card className="border-[#E5E7EB] hover:border-[#0F766E] hover:shadow-md transition-all bg-white h-full">
                      <CardContent className="p-6">
                        ...
                        <h3 className="font-black text-lg text-slate-900 mb-2 group-hover:text-[#0F766E] transition-colors">
                          {r.name}
                        </h3>
                        <p className="text-sm text-[#64748B] line-clamp-2 mb-4">
                          {r.shortDescription}
                        </p>
                        <span className="text-[#0F766E] text-sm font-bold flex items-center gap-1">
                          اعرف المزيد
