...
export function FloatingActions() {
  ...
  return (
    <>
      ...
      {/* LEFT - Call Button (Fixed) */}
      <a
        href={`tel:${siteConfig.phone}`}
        className={`fixed bottom-4 md:bottom-6 left-4 md:left-6 z-40 group transition-all duration-500 ${
          isVisible
      ...
      >
        {showPulse && isVisible && (
          <>
            ...
            <span
              className="absolute inset-0 rounded-full bg-[#E85D04] opacity-20 animate-ping"
        ...
        {/* Tooltip - Desktop */}
        <div className="hidden md:block absolute top-1/2 -translate-y-1/2 left-full ml-3 bg-[#1C1C1C] text-white text-xs font-black px-3 py-1.5 rounded-lg shadow-lg opacity-0 group-hover:opacity-100 transition-all whitespace-nowrap pointer-events-none">
          <span dir="ltr">{siteConfig.phone}</span>
          <div className="absolute top-1/2 -translate-y-1/2 right-full w-0 h-0 border-t-4 border-b-4 border-r-4 border-transparent border-r-[#1C1C1C]" />
        </div>
      </a>

      {/* RIGHT - WhatsApp Button (Fixed) */}
      <button
        onClick={() => setShowWidget(!showWidget)}
        className={`fixed bottom-4 md:bottom-6 right-4 md:right-6 z-40 group transition-all duration-500 ${
          isVisible
            ? "translate-y-0 opacity-100 pointer-events-auto"
            : "translate-y-20 opacity-0 pointer-events-none"
        }`}
      ...
      >
        ...
        {/* Tooltip - Desktop */}
        <div className="hidden md:block absolute top-1/2 -translate-y-1/2 right-full mr-3 bg-[#1C1C1C] text-white text-xs font-black px-3 py-1.5 rounded-lg shadow-lg opacity-0 group-hover:opacity-100 transition-all whitespace-nowrap pointer-events-none">
          {showWidget ? "إغلاق" : "دردشة واتساب"}
          <div className="absolute top-1/2 -translate-y-1/2 left-full w-0 h-0 border-t-4 border-b-4 border-l-4 border-transparent border-l-[#1C1C1C]" />
        </div>
      </button>
    </>
  );
